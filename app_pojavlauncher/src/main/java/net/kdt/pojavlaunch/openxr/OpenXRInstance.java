...
public class OpenXRInstance implements AutoCloseable {
    private static final Logger LOGGER = LogManager.getLogger("pojav");
        ...
        while (event != null) {
            switch (event.type()) {
                ...
                case XR10.XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED: {
                    return PojavLauncherPlayClient.OPEN_XR_STATE.session.stateChanged(XrEventDataSessionStateChanged.create(event.address()));
                }
    ...
    @Override
    public void close() {
        String session = PojavLauncherPlayClient.OPEN_XR_STATE.session;
        if (session != null) {
            session.close();
        }
